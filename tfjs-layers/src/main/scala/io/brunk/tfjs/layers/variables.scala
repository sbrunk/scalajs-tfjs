...
class LayerVariable protected () extends js.Object {
  def this(`val`: TensorND, dtype: DataType = ???, name: String = ???, trainable: Boolean = ???, constraint: Constraint = ???) = this()
...
object Variables extends js.Object {
  def variable(x: TensorND, dtype: DataType = ???, name: String = ???, constraint: Constraint = ???): LayerVariable = js.native
  ...
  def batchSetValue(variablesAndValues: js.Array[js.Tuple2[LayerVariable, TensorND]]): Unit = js.native
}
